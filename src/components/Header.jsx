  ...
  return (
    <header className='flex flex-col md:flex-row min-h-[17%] md:h-[15%] justify-between items-start md:items-center py-0 md:py-1'>
        ...
        <div className='relative flex items-center basis-[80%] w-[100%] mb-2 md:mb-0'>
            ...
            <input type='text' placeholder='Search' className='outline-none border-gray-300 rounded-3xl w-[100%] md:w-[90%] border-2 h-10 px-12 py-2' value={searchVal} onChange={(e) => {setSearchVal(e.target.value)}} />
        </div>
        <select className='basis-[10%] border-gray-300 border-2 h-10 rounded-3xl p-2 mr-5 outline-none' value={filter} onChange={(e) => {setFilter(+e.target.value)}}>
